...
public class Play extends JApplet  implements Runnable,MouseMotionListener {	
	Thread thread;
	...
	Timer timerFrog = new Timer(1, new ActionListener (){
    	public void actionPerformed(ActionEvent e){
	...
	Timer timerBall = new Timer(600, new ActionListener (){ //600
    	public void actionPerformed(ActionEvent e){
	...
	public void init() { 
		...
		for(int i=0; i<allBall;i++) {
			...
			if(i<10) {
			balls[i].changeImg(4);
			} else  		balls[i].changeImg(1);
			
		}
		
	}
	public void checkMaxPoint() {
	//	System.out.println("CHECKMAX color frog "+frog.color);
		if(checkAuto) {
		max=0;
		min=0;
		colorMax=0;
		for(int i=0;i<rear;i++) {
			if(frog.color==balls[i].color) {
					max++;
	...
	/*	angle = Math.atan2(mouseY1- this.getHeight()/2  , mouseX1- this.getWidth()/2);
		 ...
		 ballShoot.y+=(30*(Math.sin(angle)));*/
				...
				angle = Math.atan2(balls[colorMax].y- this.getHeight()/2,balls[colorMax].x - this.getWidth()/2);
			 ballShoot.x+=(30*(Math.cos(angle)));
			 ballShoot.y+=(30*(Math.sin(angle)));
	...
	public void moveBall(Ball ball) {
		if(ball.x <= 704 && ball.y==65) {
			...
			ball.y+=16;
			 ...
			 ball.x=96;
			 ball.y-=16;
			 if(balls[first].y<=370 && balls[first].x==96) {
				 check=2;
	...
	public void update(Graphics g) {
		...
		if(check==1 && rear!=0) {
			...
			load.repaint();	
		else if(check==2 || rear==0){
			timerFrog.stop();
			timerBall.stop();
			add(home);
			check=0;
			rear=10;
			allBall=300; // rear show ball 50 but all balls are 300
			max=0;
			colorMax=0;
			min=0;
	...
	public void roFrog() {
		currentAngle =-1*Math.atan2(balls[colorMax].x-frog.getX(),balls[colorMax].y-frog.getY()) / Math.PI * 180;
